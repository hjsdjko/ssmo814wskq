	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="适用场景" prop="shiyongchangjing">
					<el-select :disabled="ro.shiyongchangjing" v-model="ruleForm.shiyongchangjing" placeholder="请选择适用场景" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="适用场景" prop="shiyongchangjing">
					<el-input v-model="ruleForm.shiyongchangjing"
						placeholder="适用场景" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="单次限购" prop="onelimittimes">
					<el-input v-model="ruleForm.onelimittimes" placeholder="单次限购" clearable  :readonly="ro.onelimittimes"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="单次限购" prop="onelimittimes">
					<el-input v-model="ruleForm.onelimittimes" placeholder="单次限购" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="鲜花库存" prop="alllimittimes">
					<el-input v-model="ruleForm.alllimittimes" placeholder="鲜花库存" clearable  :readonly="ro.alllimittimes"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="鲜花库存" prop="alllimittimes">
					<el-input v-model="ruleForm.alllimittimes" placeholder="鲜花库存" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="促销详情" prop="cuxiaoxiangqing">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.cuxiaoxiangqing" label="促销详情" prop="cuxiaoxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.cuxiaoxiangqing"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				this.ruleForm.alllimittimes='100'
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.cuxiaoxiangqing = this.ruleForm.cuxiaoxiangqing.replace(reg,'../../../ssmo814wskq/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tejiacuxiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tejiacuxiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
