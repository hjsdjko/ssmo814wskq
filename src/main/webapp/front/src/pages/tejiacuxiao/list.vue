...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"2px solid #dbd9f4","padding":"10px","margin":"10px 0 0 0","alignItems":"center","borderRadius":"8px","flexWrap":"wrap","background":"#f8f8fc","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>鲜花产地</div>
        <el-input v-model="formSearch.xianhuachandi" placeholder="鲜花产地" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>适用场景</div>
        <el-select v-model="formSearch.shiyongchangjing" placeholder="请选择适用场景" :clearable="true">
          <el-option v-for="(item, index) in shiyongchangjingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"10px"}'>
	  ...
	  <el-button v-if="isAuth('tejiacuxiao','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#666","borderRadius":"6px","background":"linear-gradient(90deg, rgba(255,233,100,1) 0%, rgba(194,248,126,1) 29%, rgba(181,233,252,1) 61%, rgba(246,172,218,1) 100%)","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/tejiacuxiaoAdd')"><i v-if="true" :style='{"color":"#666","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"border":"2px solid #dbd9f4","margin":"20px 0 0 0","borderRadius":"8px","background":"#fff"}'>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"20px 32px 20px 20px","flexWrap":"wrap","justifyContent":"space-between","display":"block","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0","margin":"0 0 20px 2%","borderRadius":"8px","background":"#f6f6f6","display":"inline-block","width":"23%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"calc(100% - 24px)","margin":"12px","objectFit":"contain","borderRadius":"8px","display":"inline-block","height":"240px"}' v-if="item.xianhuatupian && item.xianhuatupian.substr(0,4)=='http'" :src="item.xianhuatupian" class="image" />
				<img :style='{"width":"calc(100% - 24px)","margin":"12px","objectFit":"contain","borderRadius":"8px","display":"inline-block","height":"240px"}' v-else :src="baseUrl + (item.xianhuatupian?item.xianhuatupian.split(',')[0]:'')" class="image" />
				<div class="item-info" :style='{"padding":"0px","overflow":"hidden","borderRadius":"0 0 8px 8px","background":"#f6f6f6","display":"inline-block","width":"100%","height":"auto"}'>
					<div :style='{"padding":"0 0 0 12px","margin":"0px 24px 4px 0","whiteSpace":"nowrap","overflow":"hidden","color":"#333","borderRadius":"0 20px 20px 0","background":"#ede9f6","lineHeight":"32px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.xianhuamingcheng}}</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
        this.$http.get('option/xianhuafenlei/xianhuafenlei').then(res => {
          if (res.data.code == 0) {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get('tejiacuxiao/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/tejiacuxiaoDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
