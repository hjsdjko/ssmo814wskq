...
<div class="home-preview" :style='{"width":"1200px","margin":"10px auto","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}'>
	<el-row type="flex" :gutter="5" justify="center" style="width: 100%">
		...
		<el-col :span="3" v-if="queryIndex==0">
			<el-button type="primary" @click="search('xianhuaxinxi')">搜索</el-button>
		</el-col>
	</el-row>
...
<div class="recommend" :style='{"padding":"0px 10px 10px 10px","margin":"20px 0px 0px","borderColor":"#dbd9f4","borderRadius":"8px","background":"#fff","borderWidth":"2px 1px 1px 1px","width":"100%","borderStyle":"solid","height":"auto","order":"3"}'>
    ...
    <div class="title" :style='{"width":"260px","margin":"0px auto","lineHeight":"42px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20221105/bb1ea9437beb4e1da8fcd1583db2f111.png) no-repeat,radial-gradient(circle, rgba(219,217,244,1) 0%, rgba(181,177,240,1) 100%)","height":"42px"}'>
		<span :style='{"color":"#333","fontSize":"22px"}'>鲜花信息推荐</span>
	</div>
	...
	<div class="list list3 index-pv1">
		<div :style='{"width":"100%","padding":"10px","margin":"12px 0 0px 0","borderRadius":"8px","background":"#fff","height":"auto"}' class="swiper-container" id="recommendxianhuaxinxi">
			<div class="swiper-wrapper">
				<div class="swiper-slide animation-box" :style='{"border":"0","cursor":"pointer","padding":"0 0 8px 0","borderRadius":"8px","background":"rgba(242, 242, 242,.6)","fontSize":"0","position":"relative"}' v-for="(item,index) in xianhuaxinxiRecommend" :key="index" @click="toDetail('xianhuaxinxiDetail', item)">
					<img :name="item.id" :style='{"border":"1px solid #eee","width":"100%","borderRadius":"8px 8px 0 0","objectFit":"contain","background":"#f8f8fc","height":"220px"}' v-if="preHttp(item.xianhuatupian)" :src="item.xianhuatupian.split(',')[0]" alt="" />
					<img :name="item.id" :style='{"border":"1px solid #eee","width":"100%","borderRadius":"8px 8px 0 0","objectFit":"contain","background":"#f8f8fc","height":"220px"}' v-else :src="baseUrl + (item.xianhuatupian?item.xianhuatupian.split(',')[0]:'')" alt="" />
					<div class="line1" :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"32px","color":"#333","textAlign":"center","background":"#f8f8fc"}'>{{item.xianhuamingcheng}}</div>
				</div>
			</div>
			<!-- 如果需要导航按钮 -->
	...
	<div @click="moreBtn('xianhuaxinxi')" :style='{"border":"0","cursor":"pointer","margin":"10px auto","borderRadius":"30%","textAlign":"center","background":"radial-gradient(circle, rgba(231,230,244,1) 0%, rgba(219,217,244,1) 100%),#dbd9f4","display":"block","width":"18%","lineHeight":"32px"}'>
...
<div class="lists" :style='{"margin":"20px 0 0","borderColor":"#dbd9f4","borderRadius":"8px","flexWrap":"wrap","background":"#fff","borderWidth":"1px 1px 1px 2px","display":"flex","borderStyle":"solid","justifyContent":"space-between","order":"4"}'>
	...
	<div class="title" :style='{"margin":"auto 0","alignItems":"center","textAlign":"center","background":"url(http://codegen.caihongy.cn/20221105/8dd5408f3db747ada55008950b9a3902.png) no-repeat,radial-gradient(circle, rgba(219,217,244,1) 0%, rgba(181,177,240,1) 100%)","display":"flex","width":"42px","lineHeight":"1.1","height":"260px"}'>
	  <span :style='{"color":"#333","fontSize":"22px"}'>特价促销展示</span>
	</div>
	...
	<div class="list list1 index-pv1" :style='{"width":"calc(100% - 42px)","padding":"10px 10px 0 10px","borderRadius":"8px","background":"#fff","height":"auto"}'>
		<div :style='{"cursor":"pointer","padding":"0 0 8px 0","margin":"10px 1%","borderRadius":"8px","background":"rgba(242, 242, 242,.6)","display":"inline-block","width":"18%","position":"relative","height":"auto"}' v-for="(item,index) in tejiacuxiaoList" :key="index" @click="toDetail('tejiacuxiaoDetail', item)" class="list-item animation-box">
			<img :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","borderRadius":"8px 8px 0 0","display":"block","height":"210px"}' v-if="preHttp(item.xianhuatupian)" :src="item.xianhuatupian.split(',')[0]" alt="" />
			<img :style='{"border":"1px solid #eee","width":"100%","objectFit":"contain","borderRadius":"8px 8px 0 0","display":"block","height":"210px"}' v-else :src="baseUrl + (item.xianhuatupian?item.xianhuatupian.split(',')[0]:'')" alt="" />
			<div class="name line1" :style='{"padding":"0px","color":"#666","textAlign":"center","background":"#f8f8fc","lineHeight":"24px","fontSize":"14px","height":"24px"}'>{{item.xianhuamingcheng}}</div>
		</div>
	</div>
	...
	<div @click="moreBtn('tejiacuxiao')" :style='{"border":"0","cursor":"pointer","margin":"10px auto","borderRadius":"30%","textAlign":"center","background":"radial-gradient(circle, rgba(231,230,244,1) 0%, rgba(219,217,244,1) 100%),#dbd9f4","display":"block","width":"18%","lineHeight":"32px"}'>
		<span :style='{"color":"#666","fontSize":"14px"}'>查看更多</span>
		<i v-if="true" :style='{"color":"#666","fontSize":"14px"}' class="el-icon-d-arrow-right"></i>
	</div>
		...
		getList() {
          ...
          if(localStorage.getItem('Token')) {
			...
			this.$http.get(autoSortUrl, {params: {
				...
				if (res.data.code == 0) {
					this.xianhuaxinxiRecommend = res.data.data.list;
					...
					this.$nextTick(() => {
						new Swiper('#recommendxianhuaxinxi', options)
					})
			...
			this.$http.get('tejiacuxiao/list', {params: {
				page: 1,
				limit: 5,
			}}).then(res => {
				if (res.data.code == 0) {
					this.tejiacuxiaoList = res.data.data.list;
